...
const actions = {
    ...
    async getOne(cxt, id) {
        let data = await client.get('roles/' + id);
        cxt.commit("SET_ONE", data.data);
        return data;
    },
    async create(cxt, payload) {
        let { data } = payload.id ? await client.put('roles/' + payload.id, payload) : await client.post('roles', payload);
    ...
    async del(cxt, id) {
        let { data } = await client.delete('roles/'+id);
        cxt.commit("SET_ONE", data);
        return data;
    },
    async getAllP({ commit }, params = {}) {
        ...
        let data  = await client.get(`roles/search?${queryParams}`);
        commit("SET_ALL", data)
        return data;
    },
};
